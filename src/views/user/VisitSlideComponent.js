                ...
                data?.map(( item , index ) => { return (
                    ...
                    <div className="col-md-6 mb-4" key={index}>
                        <div className="d-flex flex-column shadow-sm bg-white rounded-3 border">
                            <div className="d-flex p-3 border-bottom text-secondary"> Rendez-vous de visit </div>
                            <div className="p-3 border-bottom text-muted text-secondary">
                            ...
                            <div className="d-flex justify-content-between px-3 py-3">
                                <button className="btn btn-outline-secondary" onClick={ ()=> openLocalisationModal(item.Property.longitude, item.Property.latitude) }> <i className="bi bi-geo-alt-fill me-2"></i> Localisation </button>
                                {
                                   ( item.Property.isbooked == 1) ? (<span className="text-secondary px-2 py-1 border rounded-2 small"> propriété prise </span>) : (<button className="btn btn-outline-secondary" onClick={ ()=> openValidationModal(item.Property.guaranty, item.Property.id) } > Validation <i className="bi bi-arrow-right ms-2"></i>   </button>) 
                                }   
                            </div>
                        </div>
                    </div>  
